...
impl Drop for DownloadStateGuard<'_> {
    fn drop(&mut self) {
        if self.active {
            self.manager.clear_download_state(self.model_id);
        }
    }
}
...
impl ModelManager {
    ...
    fn model_broker_url(&self) -> String {
        match std::env::var(MODEL_BROKER_URL_ENV) {
            Ok(url) if !url.trim().is_empty() => url,
            _ => DEFAULT_MODEL_BROKER_URL.to_string(),
        }
    }

    fn clear_download_state(&self, model_id: &str) {
        {
            let mut models = self.available_models.lock().unwrap();
    ...
    async fn fetch_download_contract(
        &self,
        client: &reqwest::Client,
        model_id: &str,
    ) -> Result<DownloadBrokerResponse> {
        let response = client
            .get(self.model_broker_url())
            .query(&[("model_id", model_id)])
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Failed to request model broker token: {}", e))?;

        let status = response.status();
        if !status.is_success() {
            return Err(anyhow::anyhow!(
                "Model broker request failed with status {}",
                status
            ));
        }

        let broker_response = response
            .json::<DownloadBrokerResponse>()
            .await
            .map_err(|e| anyhow::anyhow!("Failed to parse model broker response: {}", e))?;

        if broker_response.model_id != model_id {
            return Err(anyhow::anyhow!(
                "Model broker returned mismatched model_id {} for {}",
                broker_response.model_id,
                model_id
            ));
        }

        if broker_response.download_url.trim().is_empty() {
            return Err(anyhow::anyhow!(
                "Model broker returned an empty download URL"
            ));
        }

        if broker_response.authorization_token.trim().is_empty() {
            return Err(anyhow::anyhow!(
                "Model broker returned an empty authorization token"
            ));
        }

        if broker_response.expires_at_unix == 0 {
            return Err(anyhow::anyhow!(
                "Model broker returned an invalid expiration"
            ));
        }
    ...
    fn build_download_request<'a>(
        ...
        broker_response: &'a DownloadBrokerResponse,
        resume_from: u64,
    ) -> reqwest::RequestBuilder {
        let mut request = client.get(&broker_response.download_url).header(
            reqwest::header::AUTHORIZATION,
            &broker_response.authorization_token,
        );
    ...
    async fn send_download_request_with_refresh(
        &self,
        client: &reqwest::Client,
        model_id: &str,
        broker_response: &mut DownloadBrokerResponse,
        resume_from: u64,
    ) -> Result<reqwest::Response> {
        let mut response = self
            .build_download_request(client, broker_response, resume_from)
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Failed to request model download: {}", e))?;
        ...
        if matches!(
            ...
            warn!(
                "Download authorization expired for model {}, refreshing broker token",
                model_id
            );

            *broker_response = self.fetch_download_contract(client, model_id).await?;
            ...
            response = self
                ...
                .map_err(|e| {
                    anyhow::anyhow!(
                        "Failed to request model download after refreshing authorization: {}",
                        e
                    )
                })?;
        }
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<()> {
        ...
        model_info
            .url
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("No download source configured for model"))?;
        ...
        let mut resume_from = if partial_path.exists() {
            ...
            info!("Starting fresh download of model {}", model_id);
            0
        };
        ...
        let mut download_guard = DownloadStateGuard::new(self, model_id);

        // Create HTTP client and fetch an authenticated download contract
        let client = reqwest::Client::new();
        let mut broker_response = self.fetch_download_contract(&client, model_id).await?;
        let mut response = self
            .send_download_request_with_refresh(
                &client,
                model_id,
                &mut broker_response,
                resume_from,
        ...
        if resume_from > 0 && response.status() == reqwest::StatusCode::OK {
            ...
            response = self
                .send_download_request_with_refresh(&client, model_id, &mut broker_response, 0)
                .await?;
        }
